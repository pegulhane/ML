...
class Model:
  ...
  def run_test(self, batch_size):
    train_X = np.asmatrix([3.3, 4.4, 5.5, 6.71, 6.93, 4.168, 9.779, 6.182, 7.59, 2.167,
                              7.042, 10.791, 5.313, 7.997, 5.654, 9.27, 3.1, 3.1]).reshape(-1, batch_size)
    train_Y = np.asmatrix([1.7, 2.76, 2.09, 3.19, 1.694, 1.573, 3.366, 2.596, 2.53, 1.221,
                              2.827, 3.465, 1.65, 2.904, 2.42, 2.94, 1.3, 1.3]).reshape(-1, batch_size)
      ...
      for i in range(50):
        for (x, y) in zip(train_x, train_y):
          pred_loss, _ = session.run([self.loss, self.optimiser],
                                     feed_dict={self.x: np.transpose(x),
